    ...
    def processFiles(self):
        ...
        for filename in os.listdir(self.folder_path):
            if filename.endswith('.par'):
                ...
                try:
                    df = pd.read_parquet(file_path, engine='auto', columns=['FCC_34sysState'],
                                                  storage_options=None, use_nullable_dtypes=False)
                    if 'FCC_34sysState' in df.columns:
                        counts = df['FCC_34sysState'].value_counts()
                        count_5 += counts.get(5, 0)
                        count_6 += counts.get(6, 0)
                        count_11 += counts.get(11, 0)
                    else:
                        print(f"Column 'FCC_34sysState' not found in file: {file_path}")
